# AGENTS.md — 原神 CP 壁纸套件17 · AI 自动安装指引(AI 助手必读)

> **给 AI 的话**: 当用户把本仓库地址发给你, 并说「安装原神CP17壁纸 / 原神CP17 /
> 奥黛塔沃雅妮莎米提亚阿罗夏壁纸 / 换个壁纸」时, **你就是安装器**。
> 请直接替用户执行下面的步骤, 不要让用户自己敲命令。所有命令都是幂等的。

## 这套东西是什么

**原神 CP 壁纸套件 17 · 奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏** —— **单张素材**壁纸包。

只有一张插画(**2×2 四宫格**校服合影: 左上奥黛塔、右上沃雅妮莎、左下米提亚、
右下阿罗夏), 但提供**三种摆法**:

| 模式 id | 名称 | 说明 |
|---|---|---|
| `showall1` | 完整 | **默认**。等比放进同色纯色底, **一个像素都不裁**, 左右留边 |
| `single1` | 卡片式 | 模糊填充背景 + 居中圆角卡片, 构图完整不裁切 |
| `cover1` | 满屏 | cover 铺满整屏, 无边框 |

素材已打进发行包, **离线可用**。跨平台 Windows / macOS / Linux。

> **为什么默认是「完整」**: 素材是 1104×1480 的竖图(ar 0.746),
> 铺满 16:9 必须上下裁掉大半, 满屏取景窗只占源高 42%, 会从四宫格中间横切,
> 四位角色都看不全。所以默认走「完整」, 四格都看得见。

> 原图左上角保留有画师署名 `@Christar_Eve` —— 这是**有意保留**的,
...
pip install "https://github.com/WPH666-py/Genshen-skin-CP17/raw/main/dist/genshen_skin_cp17-0.1.0-py3-none-any.whl"

# B. 源码
git clone https://github.com/WPH666-py/Genshen-skin-CP17.git "$HOME/Genshen-skin-CP17"
cd "$HOME/Genshen-skin-CP17"
```

验证: `genshen-cp17 info` 应打印素材路径与屏幕分辨率。
**若该命令不存在**, 说明 Scripts 目录不在 PATH, 改用 `python -m genshen_skin_cp17.engine.cli info`。
...
源码形态等价命令: `python -m genshen_skin_cp17.engine.autoinstall`
...
genshen-cp17 all        # 一次生成 3 张到 ~/.genshen-cp17/wallpapers
```

### B. VSCode / Trae / CodeX / Cursor / Windsurf

```bash
genshen-cp17-install --only vscode
```

手动等价:
`code --install-extension <仓库>/vscode/genshen-skin-cp17-0.1.0.vsix`

无网时: 把 `vscode/` 整个目录复制到
`%USERPROFILE%\.vscode\extensions\wp666.genshen-skin-cp17-0.1.0\`, 然后重启编辑器。

装完提示用户: 活动栏 **原神CP17** 图标 → 皮肤画廊 **3 张卡片**分别对应三种摆法。

### C. PyCharm / WebStorm / IntelliJ

```bash
genshen-cp17 all --out "$HOME/GenshenCP17-Backgrounds"
```

生成 3 张。再引导用户:
Settings → Appearance & Behavior → Appearance → **Background Image**
→ 点 `+` 选图片(编辑器区推荐 `showall1-*.jpg`, 四格全见、留白最多)。
细节见 [ide/jetbrains/README.md](ide/jetbrains/README.md)。

### D. DeepKing 界面皮肤(深度适配)

```bash
genshen-cp17 deepking           # 生成皮肤 JSON + 离线预览, 并打印接入步骤
genshen-cp17 deepking --check   # 校验仓库是否满足 DeepKing 转换契约
genshen-cp17 deepking --what    # 显示 DeepKing 会从仓库里提取到什么
```

给用户的两句话:

1. 打开 DeepKing → **设置 → 界面皮肤 / UI Skin**;
2. 粘贴 `https://github.com/WPH666-py/Genshen-skin-CP17` → 生成自定义皮肤。

也可离线接入: 用 `genshen-cp17 deepking` 生成的
`~/.genshen-cp17/deepking/genshen-cp17.skin.json`。
...
| `src/client/genshen-cp17.module.css` | 配色变量 `--名: #hex`; 变量名需含 `bg-base`、`label-primary`、`brand-primary`、`border-l2` 等关键词 |
| `assets/background/mascot-cp17-light.jpg` / `-dark.jpg` | 编辑区右下角水印, 同一张素材的两段取景(左上奥黛塔 / 中下米提亚) |
...
```json
{
  "mcpServers": {
    "genshen-cp17": {
      "command": "python",
      "args": ["-m", "genshen_skin_cp17.engine.mcp_server"]
    }
  }
}
```
...
```bash
genshen-cp17 switcher    # 可视化切换器: 左侧预览三种摆法, 右侧一键应用
genshen-cp17 pet         # 桌面桌宠: 左键拖动, 右键菜单, Esc 退出
...
- 生成物放在 `~/.genshen-cp17/`(Windows: `C:\Users\<你>\.genshen-cp17`), **不会**改动仓库文件。
