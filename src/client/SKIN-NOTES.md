...
本目录的 `genshen-cp17.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp17.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp17 deepking          # 导出 ~/.genshen-cp17/deepking/*.skin.json
  ```
...
```bash
genshen-cp17 deepking --check    # 应输出「通过」
genshen-cp17 deepking --what     # 看转换器实际会取到什么值
```
...
本套件只有一张素材, 取两段不同构图作水印）：

- `mascot-cp17-light.jpg` —— 取左侧双人(沃雅妮莎+沃雅妮莎), 配浅色界面
- `mascot-cp17-dark.jpg` —— 取右侧双人(沃雅妮莎+绫华), 配深色界面
