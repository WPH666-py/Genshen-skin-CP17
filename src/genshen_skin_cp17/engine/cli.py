...
本套件只有一张素材(奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏), 提供三种摆法:
  ...
  genshen-cp17 all --out DIR   # 生成到指定目录(JetBrains 背景图用)
  genshen-cp17 cycle 30        # 每 30 分钟自动随机换壁纸
  genshen-cp17 switcher        # 打开可视化切换器
  genshen-cp17 pet             # 启动桌面桌宠
  genshen-cp17 deepking        # 生成 DeepKing 界面皮肤 + 离线预览
  genshen-cp17 copy            # 只合成不设置
...
def build_parser():
    ...
    # 每一种模式都注册成子命令。
    # argparse 的子命令是"贪婪"的: 一旦注册了子解析器, `genshen-cp17 cover`
    ...
    # 覆盖掉子解析器 set_defaults 设的 mode, 于是 `genshen-cp17 cover2` 反而
