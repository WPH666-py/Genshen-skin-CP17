...
def pkg_root():
    """包目录 .../genshen_skin_cp17(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp17.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eceef5;
  --bg-layer-3: #dbe0ee;
  --label-primary: #1e2338;
  --label-secondary: #485072;
  --label-tertiary: #7b83a3;
  --brand-primary: #6b79ad;
  --accent: #6b79ad;
  --primary: #6b79ad;
  --interactive-bg-hover-solid: #e2e6f2;
  --interactive-bg-active: #ccd4e8;
  --border-l1: #dfe3f0;
  --border-l2: #c6cde2;
  --border: #c6cde2;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP17</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP17", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP17", mascot.get("dark")),
    }
