# -*- coding: utf-8 -*-
"""
原神 CP 壁纸套件 17 —— MCP (Model Context Protocol) 服务器
...
注册示例(任选其一):
  claude mcp add genshen-cp17 -- genshen-cp17-mcp
  # 或写入 .mcp.json / mcp_servers.json:
  {"mcpServers": {"genshen-cp17": {"command": "genshen-cp17-mcp", "args": []}}}
...
TOOLS = [
    ...
    },
    {
        ...
        "inputSchema": {
            "type": "object",
            "properties": {
                "out_dir": {"type": "string", "description": "输出目录, 默认 ~/.genshen-cp17/wallpapers"},
...
def tool_list_wallpapers(_args):
    ...
    if sc.count() > 1:
        ...
        lines += ["", "单张素材 x 三种摆法 = 3 种壁纸。换图用序号, 换摆法用 single/cover/showall 前缀。"]
    return _ok_text("\n".join(lines))
