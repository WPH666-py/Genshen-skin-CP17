...
PACKS = {
    ...
    "CP9": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP9"),
        "pkg": ("Genshen-skin-CP9", "src", "genshen_skin_cp9"),
        "flat": False,
        "slug": "genshen_skin_cp9",
    },
    "CP10": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP10"),
        "pkg": ("Genshen-skin-CP10", "src", "genshen_skin_cp10"),
        "flat": False,
        "slug": "genshen_skin_cp10",
    },
    "CP11": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP11"),
        "pkg": ("Genshen-skin-CP11", "src", "genshen_skin_cp11"),
        "flat": False,
        "slug": "genshen_skin_cp11",
    },
    "CP12": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP12"),
        "pkg": ("Genshen-skin-CP12", "src", "genshen_skin_cp12"),
        "flat": False,
        "slug": "genshen_skin_cp12",
    },
    "CP13": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP13"),
        "pkg": ("Genshen-skin-CP13", "src", "genshen_skin_cp13"),
        "flat": False,
        "slug": "genshen_skin_cp13",
    },
    "CP14": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP14"),
        "pkg": ("Genshen-skin-CP14", "src", "genshen_skin_cp14"),
        "flat": False,
        "slug": "genshen_skin_cp14",
    },
    "CP16": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP16"),
        "pkg": ("Genshen-skin-CP16", "src", "genshen_skin_cp16"),
        "flat": False,
        "slug": "genshen_skin_cp16",
    },
    "CP17": {
        "repo": os.path.join(WORKSPACE, "Genshen-skin-CP17"),
        "pkg": ("Genshen-skin-CP17", "src", "genshen_skin_cp17"),
        "flat": False,
        "slug": "genshen_skin_cp17",
    },
}
...
def engine_dir(key):
    """引擎文件所在目录: CP1 是包根, CP2~CP16 是包内的 engine/。"""
    v = PACKS[key]
...
def _role_of(key):
    ...
    return {"CP2": "odette_voj", "CP3": "cp3_trio",
            ...
            "CP8": "cp8_pair", "CP9": "cp9_pair", "CP10": "cp10_pair", "CP11": "cp11_pair",
            "CP12": "cp12_pair", "CP13": "cp13_pair", "CP14": "cp14_trio",
            "CP15": "cp15_pair", "CP16": "cp16_pair",
            "CP17": "cp17_quartet"}.get(key)
...
def normalize(text, src_slug, dst_slug, dst_key=None, src_key=None, src_repo=None,
    ...
    还要改写「常量模块」的导入, 各套件结构不同:
      ...
      CP4: from ..characters import cp17_quartet as C
    ...
    CN = {
        ...
        "CP3": ("原神 CP 壁纸套件 3", "原神CP3", "奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏", "~/.genshen-cp3"),
        "CP4": ("原神 CP 壁纸套件 17", "原神CP17", "奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏", "~/.genshen-cp17"),
    }
    ...
    # 注意: 源与目标的角色模块名可能不同(cp17_quartet vs odette_voj),
...
def main():
    ap = argparse.ArgumentParser(
        description="CP1~CP16 皮肤引擎同步工具(本机存在哪些套件就处理哪些)")
