// 原神 CP 壁纸套件 17 — 奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏 · 单张素材三种呈现 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP17.git';
const EXT_ID = 'wp666.genshen-skin-cp17';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp17').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp17').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP17'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP17') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp17', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp17.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp17.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp17' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP17') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp17.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
// 本套件只有一张素材(奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏 双人合影), 因此画廊给的是同一张的
// 三种呈现方式。ids 必须与 characters/cp17_quartet.py 的 MODES 一致。
const CARDS = [
  { mode: 'showall1', t: '四人合影 · 完整', img: 'thumb-showall1.png', d: '不裁切, 四格全见(默认)' },
  { mode: 'single1', t: '四人合影 · 卡片', img: 'thumb-single1.png', d: '模糊背景 + 居中卡片' },
  { mode: 'cover1', t: '四人合影 · 满屏', img: 'thumb-cover1.png', d: '铺满整屏(竖图会裁掉大半)' }
];
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件 17 · 奥黛塔 × 沃雅妮莎 × 米提亚 × 阿罗夏 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshencp17.setShowall', () => runCli(['showall'], '已切换: 完整'));
  reg('genshencp17.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp17.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp17.setRandom', () => runCli(['random'], '已随机换一种!'));
  reg('genshencp17.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp17.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp17.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp17.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP17 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp17.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp17')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp17.gallery', new GalleryProvider(), {
